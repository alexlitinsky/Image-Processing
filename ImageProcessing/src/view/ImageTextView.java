  ...
  public String toString() {
    ...
    for (int i = 0; i < model.getDimensions()[0]; i++) {
      for (int j = 0; j < model.getDimensions()[1]; j++) {
        try {
          String red = String.valueOf(model.getPixel(i, j).getRGB()[0]);
          String green = String.valueOf(model.getPixel(i, j).getRGB()[1]);
          String blue = String.valueOf(model.getPixel(i, j).getRGB()[2]);
